...
> 最后更新：2026-07-02（**① 增量更新：距上次(6/25)7天，走增量+窗口放宽到8天(N+1)，5个Sonnet子agent只抓6/24后新料，本轮新增16条/淘汰24条超期。② 前端空态三分支：维度「今日无新增」时不再留白，显示提示条+往期分割线，接往期最新动态。**）
...
- **【2026-07-02 新增】维度空态三分支渲染**（纯前端，普通维度 + 连锁卡 feed 通用）：① 有今日新增→🆕置顶+「往期动态」分割线+往期；② 今日无新增但有往期→「📭 今日无新增，以下为往期最新动态」提示条+往期；③ 彻底无料→「今日无新动态」。目的：维度没动静时也不留白，业务方一眼分清今日新增与往期存量。样式类 `.no-new` / `.past-sep`（详见 AGENTS.md 数据契约段）。
