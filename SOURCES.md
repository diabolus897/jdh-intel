...
## 二·五、上市公司财报 & 投资者纪要获取手册（2026-06-25 实测固化，连锁药店等竞对适用）

> 上市公司信息披露有**法定唯一渠道**，找对入口又准又快。以下三条路径均经 **Jina Reader 兜底实测可达**（`https://r.jina.ai/<目标url>` 再 WebFetch）。

### 路径 A：财务数字 → 东方财富 F10（首选，最高效）
- **入口（把代码替换即可）**：`https://emweb.eastmoney.com/PC_HSF10/NewFinanceAnalysis/Index?type=web&code=SH<6位代码>`
  - 老百姓 `SH603883`｜益丰 `SH603939`｜大参林 `SH603233`（深市用 `SZ` 前缀）。
- **实测**：直接返回营收/归母净利/毛利率/净利率/每股收益/ROE/资产负债率…**逐项带同比**，多期并列。抓数字效率远高于翻财报 PDF。
- ⚠ **口径注意**：东财「净利率」=归母净利/营收（老百姓2025=2.43%）；若你要的是含少数股东的口径会略不同。**写进卡片前确认口径一致**，多期用同一来源同一口径。

### 路径 B：投资者纪要/业绩说明会 → 巨潮资讯（法定权威，纪要原文）
- **正确入口=全文检索**：`http://www.cninfo.com.cn/new/fulltextSearch?keyWord=<公司名> <关键词>`
  - 例：`keyWord=老百姓 投资者关系活动记录表` / `keyWord=老百姓 业绩说明会`。
- ⚠ **踩坑**：直接拼个股页 `disclosure/stock?stockCode=603939`（不带 orgId）会报「页面不存在」。**用全文检索入口，不要拼 stockCode 个股页。**
- **实测命中真实纪要**：老百姓《机构投资者交流活动会议纪要(2026年5月)》(2026-06-05)、《关于召开2025年度暨2026年第一季度业绩说明会的公告》(2026-06-22)。纪要里有**未来战略/举措**，正是 `financials.strategy` 与 `memo` 的料。
- 全文检索返回0条 ≠ 抓取失败，可能是**该公司当期确实没开纪要**（如益丰本窗口仅可转债/分红公告）——属实即 `strategy/memo` 留空，不编造。

### 路径 C：业绩说明会（沪市）→ 上证路演中心
- `https://roadshow.sseinfo.com/`（沪市业绩说明会/路演实录）。作为巨潮的补充，找说明会视频/实录。深市公司看深交所互动易/巨潮投资者关系活动记录表。

### 标准取数动作（连锁药店经营速览卡）
1. 财务数字走 **A（东财F10）**，逐项核同比、记来源 url、统一口径。
2. 覆盖省份/门店增量/战略举措/纪要走 **B（巨潮全文检索）** 的年报「管理层讨论与分析」+ 纪要原文。
3. 每项数据写进 `data.json` 前带真实 url；查不到的字段**不写该 key**，绝不编造。
4. 季度才更新（披露季后跑一次），平时连锁卡只更新动态流 items。
